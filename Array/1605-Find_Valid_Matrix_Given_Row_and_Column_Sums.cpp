class Solution {
public:
    vector<vector<int>> restoreMatrix(vector<int> &rowSum,vector<int> &colSum) 
    {
        int rows=rowSum.size(),cols=colSum.size(),cur_row=0,cur_col=0;
        vector<vector<int>> ans(rows,vector<int>(cols,0));
        while(cur_row<rows || cur_col<cols) 
        {
            if(cur_row>=rows) 
            {
                ans[rows-1][cur_col]=colSum[cur_col];
                cur_col++;
                continue;
            } 
            else if(cur_col>=cols) 
            {
                ans[cur_row][cols-1]=rowSum[cur_row];
                cur_row++;
                continue;
            }
            int value_to_put=min(rowSum[cur_row],colSum[cur_col]);
            rowSum[cur_row]-=value_to_put;
            colSum[cur_col]-=value_to_put;
            ans[cur_row][cur_col]=value_to_put;
            if(rowSum[cur_row]==0) 
            {
                cur_row++;
            }
            if(colSum[cur_col]==0) 
            {
                cur_col++;
            }
        }
        return ans;
    }
};
